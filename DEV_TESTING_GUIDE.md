...
Expected: **81 tests passing** across 6 test files:

```
tests/message-extractor.test.ts   (12 tests)
tests/session-manager.test.ts     (25 tests)
tests/cli-invoker.test.ts         (11 tests)
tests/config.test.ts              (13 tests)
tests/hub-server.test.ts           (7 tests)
tests/hub-server-sessions.test.ts (13 tests)
```
...
### 3.1 Test `gemini-agent`

In Claude Code, type:

```
Use the gemini-agent tool to answer: "What is the capital of France?"
```
...
### 3.2 Test `codex-agent`

```
Use the codex-agent tool to answer: "Write a Python function that checks if a number is prime"
```
...
```
Use the claude-agent tool to answer: "Explain the difference between a stack and a queue in one paragraph"
```
...
> **Note:** Using `claude-agent` from within Claude Code means Claude is invoking another Claude instance via the CLI. This works but is recursive — keep prompts simple.

### 3.4 Test model override

```
Use the gemini-agent tool with model "gemini-2.0-flash" to answer: "What is 2+2?"
```

**What to verify:**
- Metadata footer shows `Model: gemini-2.0-flash` (not the default)

### 3.5 Test working directory

```
Use the codex-agent tool with workingDir set to "/tmp" to answer: "List the files in the current directory"
```
...
```
Use the gemini-agent tool with timeoutMs set to 1000 to answer: "Write a 5000 word essay about the history of computing"
```
...
### 3.7 Test error handling — invalid backend

If you restricted backends with `--backends gemini,codex`, try asking Claude to use `claude-agent`. It should not be available as a tool.
...
```
Use the gemini-agent tool to answer: "Say hello"
```
...
Restart Claude Code and run `/mcp`. Only `gemini-agent` should be listed (plus session tools). `claude-agent` and `codex-agent` should not be available.
...
```
Use the gemini-agent tool to answer: "Reply with just the letter A"
```
...
```
Use the gemini-agent tool to answer: "List all countries in the world with their capitals"
```
...
```
Use the codex-agent tool to answer: "hello"
```
...
## Part 7 — Testing All Three Backends Together

This is the full integration test — use all three agents in a single conversation.

```
I want to compare how three different AI agents answer the same question.

Use gemini-agent, codex-agent, and claude-agent to each answer:
...
- Run `which claude`, `which gemini`, `which codex` to verify
